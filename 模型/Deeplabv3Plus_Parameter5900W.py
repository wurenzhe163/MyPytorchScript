...
class _ResLayer(nn.Sequential):
    ...
    def __init__(self, n_layers, in_ch, out_ch, stride, dilation, multi_grids=None):
        ...
        #multi_grids决定空洞卷积，是一个列表长度==n_layers??
        ...
        # 罗列、堆叠出所有的残差层，残差层仅第一层输入输出层数变化，仅第一层采用下采样
...
class _ImagePool(nn.Module):
    '''
    nn.AdaptiveAvgPool2d,_ConvBnReLU,interpolate(采样)
    '''
...
class _ASPP(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, rates):
        ...
        self.stages.add_module("imagepool", _ImagePool(in_ch, out_ch))   #不晓得这一层干嘛用
...
class DeepLabV3Plus(nn.Module):
    ...
    # n_blocks 是一个列表 对应参数n_layers，决定_Bottleneck 的层数
    # atrous_rates ASPP 每层的空洞数量，len(atrous_rates)代表空洞层数
    # multi_grids是一个列表决定空洞卷积，dilation * multi_grids[i] ，len(multi_grids) = n_blocks[i]
    # output_stride决定残差层_ResLayer，s,d（步长、空洞）
